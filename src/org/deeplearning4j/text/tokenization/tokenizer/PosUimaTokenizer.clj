(ns org.deeplearning4j.text.tokenization.tokenizer.PosUimaTokenizer
...
(defn ->pos-uima-tokenizer
  ...
  strip-nones - `boolean`"
  (^PosUimaTokenizer [^java.lang.String tokens ^org.apache.uima.analysis_engine.AnalysisEngine engine ^java.util.Collection allowed-pos-tags ^Boolean strip-nones]
    (new PosUimaTokenizer tokens engine allowed-pos-tags strip-nones))
...
(defn set-token-pre-processor
  ...
  ([^PosUimaTokenizer this ^org.deeplearning4j.text.tokenization.tokenizer.TokenPreProcess token-pre-processor]
