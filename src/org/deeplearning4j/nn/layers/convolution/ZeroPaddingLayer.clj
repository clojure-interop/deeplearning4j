(ns org.deeplearning4j.nn.layers.convolution.ZeroPaddingLayer
...
(defn ->zero-padding-layer
  ...
  (^ZeroPaddingLayer [^org.deeplearning4j.nn.conf.NeuralNetConfiguration conf]
    (new ZeroPaddingLayer conf)))

(defn pretrain-layer?
  "Description copied from interface: Layer

  returns: true if the layer can be pretrained (using fit(INDArray), false otherwise - `boolean`"
  (^Boolean [^ZeroPaddingLayer this]
    (-> this (.isPretrainLayer))))

(defn clear-noise-weight-params
  ""
  ([^ZeroPaddingLayer this]
...
(defn type
  ...
  (^org.deeplearning4j.nn.api.Layer$Type [^ZeroPaddingLayer this]
    (-> this (.type))))
...
(defn backprop-gradient
  ...
  (^org.nd4j.linalg.primitives.Pair [^ZeroPaddingLayer this ^org.nd4j.linalg.api.ndarray.INDArray epsilon ^org.deeplearning4j.nn.workspace.LayerWorkspaceMgr workspace-mgr]
...
(defn activate
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^ZeroPaddingLayer this ^Boolean training ^org.deeplearning4j.nn.workspace.LayerWorkspaceMgr workspace-mgr]
    (-> this (.activate training workspace-mgr))))

(defn clone
  "returns: `org.deeplearning4j.nn.api.Layer`"
  (^org.deeplearning4j.nn.api.Layer [^ZeroPaddingLayer this]
    (-> this (.clone))))
...
(defn calc-l-1
  ...
  (^Double [^ZeroPaddingLayer this ^Boolean backprop-params-only]
