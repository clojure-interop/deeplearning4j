(ns org.deeplearning4j.nn.layers.convolution.Cropping1DLayer
...
(defn ->cropping-1-d-layer
  "Constructor.

  conf - `org.deeplearning4j.nn.conf.NeuralNetConfiguration`"
  (^Cropping1DLayer [^org.deeplearning4j.nn.conf.NeuralNetConfiguration conf]
    (new Cropping1DLayer conf)))

(defn pretrain-layer?
  "Description copied from interface: Layer

  returns: true if the layer can be pretrained (using fit(INDArray), false otherwise - `boolean`"
  (^Boolean [^Cropping1DLayer this]
    (-> this (.isPretrainLayer))))

(defn clear-noise-weight-params
  ""
  ([^Cropping1DLayer this]
...
(defn type
  ...
  (^org.deeplearning4j.nn.api.Layer$Type [^Cropping1DLayer this]
    (-> this (.type))))
...
(defn backprop-gradient
  ...
  (^org.nd4j.linalg.primitives.Pair [^Cropping1DLayer this ^org.nd4j.linalg.api.ndarray.INDArray epsilon ^org.deeplearning4j.nn.workspace.LayerWorkspaceMgr workspace-mgr]
...
(defn activate
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^Cropping1DLayer this ^Boolean training ^org.deeplearning4j.nn.workspace.LayerWorkspaceMgr workspace-mgr]
    (-> this (.activate training workspace-mgr))))

(defn clone
  "returns: `org.deeplearning4j.nn.api.Layer`"
  (^org.deeplearning4j.nn.api.Layer [^Cropping1DLayer this]
    (-> this (.clone))))
...
(defn calc-l-1
  ...
  (^Double [^Cropping1DLayer this ^Boolean backprop-params-only]
