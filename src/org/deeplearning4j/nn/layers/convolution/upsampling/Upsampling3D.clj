(ns org.deeplearning4j.nn.layers.convolution.upsampling.Upsampling3D
  "3D Upsampling layer.

 Used for upsampling a 3D convolution"
  (:refer-clojure :only [require comment defn ->])
  (:import [org.deeplearning4j.nn.layers.convolution.upsampling Upsampling3D]))
...
(defn ->upsampling-3-d
  ...
  (^Upsampling3D [^org.deeplearning4j.nn.conf.NeuralNetConfiguration conf ^org.nd4j.linalg.api.ndarray.INDArray input]
    (new Upsampling3D conf input))
  (^Upsampling3D [^org.deeplearning4j.nn.conf.NeuralNetConfiguration conf]
    (new Upsampling3D conf)))

(defn score
  "Description copied from interface: Model

  returns: the score for the model - `double`"
  (^Double [^Upsampling3D this]
    (-> this (.score))))
...
(defn fit
  ...
  ([^Upsampling3D this ^org.nd4j.linalg.api.ndarray.INDArray input ^org.deeplearning4j.nn.workspace.LayerWorkspaceMgr workspace-mgr]
...
(defn gradient
  ...
  (^org.deeplearning4j.nn.gradient.Gradient [^Upsampling3D this]
    (-> this (.gradient))))
...
(defn params
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^Upsampling3D this]
    (-> this (.params))))
...
(defn activate
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^Upsampling3D this ^Boolean training ^org.deeplearning4j.nn.workspace.LayerWorkspaceMgr workspace-mgr]
...
(defn get-param
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^Upsampling3D this ^java.lang.String param]
    (-> this (.getParam param))))
...
(defn update
  ...
  ([^Upsampling3D this ^org.nd4j.linalg.api.ndarray.INDArray gradient ^java.lang.String param-type]
...
(defn set-params
  ...
  ([^Upsampling3D this ^org.nd4j.linalg.api.ndarray.INDArray params]
...
(defn backprop-gradient
  ...
  (^org.nd4j.linalg.primitives.Pair [^Upsampling3D this ^org.nd4j.linalg.api.ndarray.INDArray epsilon ^org.deeplearning4j.nn.workspace.LayerWorkspaceMgr workspace-mgr]
...
(defn type
  ...
  (^org.deeplearning4j.nn.api.Layer$Type [^Upsampling3D this]
    (-> this (.type))))
...
(defn calc-l-1
  ...
  (^Double [^Upsampling3D this ^Boolean backprop-params-only]
