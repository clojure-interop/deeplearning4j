(ns org.deeplearning4j.nn.layers.DropoutLayer
...
(defn ->dropout-layer
  ...
  (^DropoutLayer [^org.deeplearning4j.nn.conf.NeuralNetConfiguration conf ^org.nd4j.linalg.api.ndarray.INDArray input]
    (new DropoutLayer conf input))
  (^DropoutLayer [^org.deeplearning4j.nn.conf.NeuralNetConfiguration conf]
    (new DropoutLayer conf)))
...
(defn calc-l-2
  ...
  (^Double [^DropoutLayer this ^Boolean backprop-params-only]
...
(defn type
  ...
  (^org.deeplearning4j.nn.api.Layer$Type [^DropoutLayer this]
    (-> this (.type))))
...
(defn fit
  ...
  ([^DropoutLayer this ^org.nd4j.linalg.api.ndarray.INDArray input ^org.deeplearning4j.nn.workspace.LayerWorkspaceMgr workspace-mgr]
...
(defn backprop-gradient
  ...
  (^org.nd4j.linalg.primitives.Pair [^DropoutLayer this ^org.nd4j.linalg.api.ndarray.INDArray epsilon ^org.deeplearning4j.nn.workspace.LayerWorkspaceMgr workspace-mgr]
...
(defn activate
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^DropoutLayer this ^Boolean training ^org.deeplearning4j.nn.workspace.LayerWorkspaceMgr workspace-mgr]
...
(defn params
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^DropoutLayer this]
    (-> this (.params))))
