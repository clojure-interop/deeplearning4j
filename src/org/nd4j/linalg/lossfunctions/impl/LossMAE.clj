(ns org.nd4j.linalg.lossfunctions.impl.LossMAE
  "Mean absolute error loss function: L = 1/N sum_i abs(predicted_i - actual_i)
 See also LossL1 for a mathematically similar loss function (LossL1 does not have division by N, where N is output size)"
...
(defn ->loss-mae
  ...
  (^LossMAE [^org.nd4j.linalg.api.ndarray.INDArray weights]
    (new LossMAE weights))
  (^LossMAE []
    (new LossMAE )))
...
(defn op-type
  ...
  (^org.nd4j.linalg.api.ops.Op$Type [^LossMAE this]
    (-> this (.opType))))
...
(defn init-from-onnx
  ...
  ([^LossMAE this ^onnx.OnnxProto3$NodeProto node ^org.nd4j.autodiff.samediff.SameDiff init-with ^java.util.Map attributes-for-node ^onnx.OnnxProto3$GraphProto graph]
...
(defn compute-score
  ...
  returns: `double`"
  (^Double [^LossMAE this ^org.nd4j.linalg.api.ndarray.INDArray labels ^org.nd4j.linalg.api.ndarray.INDArray pre-output ^org.nd4j.linalg.activations.IActivation activation-fn ^org.nd4j.linalg.api.ndarray.INDArray mask ^Boolean average]
...
(defn do-diff
  ...
  (^java.util.List [^LossMAE this ^java.util.List f-1]
    (-> this (.doDiff f-1))))
...
(defn compute-gradient
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^LossMAE this ^org.nd4j.linalg.api.ndarray.INDArray labels ^org.nd4j.linalg.api.ndarray.INDArray pre-output ^org.nd4j.linalg.activations.IActivation activation-fn ^org.nd4j.linalg.api.ndarray.INDArray mask]
...
(defn init-from-tensor-flow
  ...
  ([^LossMAE this ^org.tensorflow.framework.NodeDef node-def ^org.nd4j.autodiff.samediff.SameDiff init-with ^java.util.Map attributes-for-node ^org.tensorflow.framework.GraphDef graph]
