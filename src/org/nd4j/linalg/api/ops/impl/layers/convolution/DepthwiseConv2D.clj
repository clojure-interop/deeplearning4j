...
(defn ->depthwise-conv-2-d
  ...
  (^DepthwiseConv2D [^org.nd4j.autodiff.samediff.SameDiff same-diff input-functions input-arrays outputs ^org.nd4j.linalg.api.ops.impl.layers.convolution.config.Conv2DConfig config]
    (new DepthwiseConv2D same-diff input-functions input-arrays outputs config))
  (^DepthwiseConv2D []
    (new DepthwiseConv2D )))
...
(defn properties-for-function
  ...
  (^java.util.Map [^DepthwiseConv2D this]
...
(defn op-name
  ...
  (^java.lang.String [^DepthwiseConv2D this]
    (-> this (.opName))))
...
(defn init-from-onnx
  ...
  ([^DepthwiseConv2D this ^onnx.OnnxProto3$NodeProto node ^org.nd4j.autodiff.samediff.SameDiff init-with ^java.util.Map attributes-for-node ^onnx.OnnxProto3$GraphProto graph]
...
(defn do-diff
  ...
  (^java.util.List [^DepthwiseConv2D this ^java.util.List f-1]
    (-> this (.doDiff f-1))))
...
(defn get-value
  ...
  (^java.lang.Object [^DepthwiseConv2D this ^java.lang.reflect.Field property]
...
(defn set-value-for
  ...
  ([^DepthwiseConv2D this ^java.lang.reflect.Field target ^java.lang.Object value]
...
(defn init-from-tensor-flow
  ...
  ([^DepthwiseConv2D this ^org.tensorflow.framework.NodeDef node-def ^org.nd4j.autodiff.samediff.SameDiff init-with ^java.util.Map attributes-for-node ^org.tensorflow.framework.GraphDef graph]
