(ns org.nd4j.linalg.api.rng.distribution.impl.OrthogonalDistribution
...
(def *-default-inverse-absolute-accuracy
  ...
  type: double"
  OrthogonalDistribution/DEFAULT_INVERSE_ABSOLUTE_ACCURACY)
...
(defn support-upper-bound-inclusive?
  ...
  (^Boolean [^OrthogonalDistribution this]
...
(defn probability
  ...
  (^Double [^OrthogonalDistribution this ^Double x-0 ^Double x-1]
...
(defn inverse-cumulative-probability
  ...
  (^Double [^OrthogonalDistribution this ^Double p]
...
(defn density
  ...
  (^Double [^OrthogonalDistribution this ^Double x]
    (-> this (.density x))))
...
(defn sample
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^OrthogonalDistribution this shape]
    (-> this (.sample shape)))
  (^Double [^OrthogonalDistribution this]
    (-> this (.sample))))
