(ns org.nd4j.linalg.cpu.nativecpu.ops.NativeOpExecutioner
  "Native operation
 executioner in c++"
  (:refer-clojure :only [require comment defn ->])
  (:import [org.nd4j.linalg.cpu.nativecpu.ops NativeOpExecutioner]))

(defn ->native-op-executioner
  "Constructor."
  (^NativeOpExecutioner []
    (new NativeOpExecutioner )))

(defn calculate-output-shape
  "op - `org.nd4j.linalg.api.ops.CustomOp`

  returns: `java.util.List<long[]>`"
  ([^NativeOpExecutioner this ^org.nd4j.linalg.api.ops.CustomOp op]
    (-> this (.calculateOutputShape op))))

(defn forget-graph
  "id - `long`"
  ([^NativeOpExecutioner this ^Long id]
    (-> this (.forgetGraph id))))
...
(defn threshold-encode
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^NativeOpExecutioner this ^org.nd4j.linalg.api.ndarray.INDArray input ^Double threshold ^java.lang.Integer boundary]
    (-> this (.thresholdEncode input threshold boundary)))
  (^org.nd4j.linalg.api.ndarray.INDArray [^NativeOpExecutioner this ^org.nd4j.linalg.api.ndarray.INDArray input ^Double threshold]
...
(defn bitmap-encode
  ...
  returns: `long`"
  (^Long [^NativeOpExecutioner this ^org.nd4j.linalg.api.ndarray.INDArray ind-array ^org.nd4j.linalg.api.ndarray.INDArray target ^Double threshold]
    (-> this (.bitmapEncode ind-array target threshold))))

(defn enable-verbose-mode
  "really-enable - `boolean`"
  ([^NativeOpExecutioner this ^Boolean really-enable]
    (-> this (.enableVerboseMode really-enable))))

(defn exec
  "Description copied from interface: OpExecutioner

  op - the index accumulation operation - `org.nd4j.linalg.api.ops.IndexAccumulation`
  dimension - the dimension/s to execute along - `int`

  returns: result - `org.nd4j.linalg.api.ndarray.INDArray`"
  (^org.nd4j.linalg.api.ndarray.INDArray [^NativeOpExecutioner this ^org.nd4j.linalg.api.ops.IndexAccumulation op ^Integer dimension]
    (-> this (.exec op dimension)))
  (^org.nd4j.linalg.api.ops.Op [^NativeOpExecutioner this ^org.nd4j.linalg.api.ops.Op op]
    (-> this (.exec op))))
...
(defn bitmap-decode
  ...
  (^org.nd4j.linalg.api.ndarray.INDArray [^NativeOpExecutioner this ^org.nd4j.linalg.api.ndarray.INDArray encoded ^org.nd4j.linalg.api.ndarray.INDArray target]
...
(defn get-environment-information
  ...
  (^java.util.Properties [^NativeOpExecutioner this]
...
(defn execute-graph
  ...
  (^java.util.Map [^NativeOpExecutioner this ^Long id ^java.util.Map map ^java.util.Map reverse-map]
...
(defn set-tad-threshold
  ...
  threshold - `int`"
  ([^NativeOpExecutioner this ^Integer threshold]
...
(defn get-custom-operations
  ...
  (^java.util.Map [^NativeOpExecutioner this]
...
(defn get-tad-manager
  ...
  (^org.nd4j.linalg.cache.TADManager [^NativeOpExecutioner this]
    (-> this (.getTADManager))))
...
(defn type
  ...
  (^org.nd4j.linalg.api.ops.executioner.OpExecutioner$ExecutionerType [^NativeOpExecutioner this]
    (-> this (.type))))
...
(defn register-graph
  ...
  ([^NativeOpExecutioner this ^Long id ^org.bytedeco.javacpp.Pointer graph]
